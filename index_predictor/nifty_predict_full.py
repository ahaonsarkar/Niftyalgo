...
class ConformalPredictionModule:
    ...
    def run(self, ensemble_wrapper, best_model, X_train, X_calib, X_test, 
                  y_train, y_calib, y_test, dates_calib, dates_test, vix_series=None):
        print("\n╔" + "═"*46 + "╗")
        print("║" + " "*8 + "CONFORMAL PREDICTION RESULTS" + " "*8 + "║")
        print("╠" + "═"*46 + "╣")
        ...
        # 1. Baseline CP
        calib_probs = ensemble_wrapper.predict_proba(X_calib)
        scores = 1 - calib_probs[np.arange(n_calib), y_calib_arr]
        
        import math
        q_hat = np.quantile(scores, min(np.ceil((n_calib + 1) * (1 - self.alpha)) / n_calib, 1.0))
        
        test_probs = ensemble_wrapper.predict_proba(X_test)
        pred_sets_baseline = test_probs >= (1 - q_hat)
        cp_results["Baseline CP"] = pred_sets_baseline
        cp_metrics.append(self.evaluate_cp(pred_sets_baseline, y_test_arr, "Baseline CP"))

        # 2. CQR (MAPIE)
        try:
            from mapie.classification import MapieClassifier
            mapie_clf = MapieClassifier(estimator=best_model, cv="prefit", method="score")
            mapie_clf.fit(X_calib, y_calib_arr)
            y_pred_mapie, y_pred_sets = mapie_clf.predict(X_test, alpha=self.alpha, include_last_label=True)
            pred_sets_cqr = y_pred_sets[:, :, 0]
        except ImportError:
            # Mapie > 0.8 removed MapieClassifier, need to check version
            try:
                ...
                print(f"║  Warning: MAPIE CQR failed ({e2}). Mocking  ║")
                pred_sets_cqr = np.ones((len(X_test), 2), dtype=bool)
        except Exception as e:
            print(f"║  Warning: MAPIE CQR failed ({e}). Mocking  ║")
            pred_sets_cqr = np.ones((len(X_test), 2), dtype=bool)
        
        cp_results["CQR (MAPIE)"] = pred_sets_cqr
        cp_metrics.append(self.evaluate_cp(pred_sets_cqr, y_test_arr, "CQR (MAPIE)"))

        # 3. Normalised CP
        pred_sets_norm = pred_sets_baseline.copy()
        if vix_series is not None:
            vix_calib = vix_series.reindex(dates_calib, method='ffill').values
            vix_test = vix_series.reindex(dates_test, method='ffill').values
            vix_calib = np.where(np.isnan(vix_calib) | (vix_calib == 0), 1e-6, vix_calib)
            vix_test = np.where(np.isnan(vix_test) | (vix_test == 0), 1e-6, vix_test)
            
            score_normalised = scores / vix_calib
            q_hat_norm = np.quantile(score_normalised, min(np.ceil((n_calib + 1) * (1 - self.alpha)) / n_calib, 1.0))
            test_scores_norm = (1 - test_probs) / vix_test[:, None]
            pred_sets_norm = test_scores_norm <= q_hat_norm
        cp_results["Normalised CP"] = pred_sets_norm
        cp_metrics.append(self.evaluate_cp(pred_sets_norm, y_test_arr, "Normalised CP"))
        ...
        for t in range(len(X_test)):
            q_hat_t = np.quantile(scores, min(np.ceil((n_calib + 1) * (1 - alpha_t)) / n_calib, 1.0))
            prob_t = test_probs[t]
            pred_set_t = prob_t >= (1 - q_hat_t)
            pred_sets_aci.append(pred_set_t)
            
            true_label_t = y_test_arr[t]
            covered = pred_set_t[true_label_t]
            
            # Update alpha
            alpha_t = alpha_t + self.gamma * (self.alpha - (1 - float(covered)))
            alpha_t = np.clip(alpha_t, 0.01, 0.5)
            alpha_sequence.append(alpha_t)
            covered_seq.append(covered)
            
        pred_sets_aci = np.array(pred_sets_aci)
        cp_results["ACI (adaptive)"] = pred_sets_aci
        cp_metrics.append(self.evaluate_cp(pred_sets_aci, y_test_arr, "ACI (adaptive)"))

        # Output Results exactly as user wanted
        print("║  Method          Coverage  Set Size  Singletons")
        for res in cp_metrics:
            m = res['Method'].ljust(15)
            cov = f"{res['Coverage']:.3f}".ljust(10)
            sz = f"{res['Avg_Set_Size']:.2f}".ljust(10)
            sgl = f"{res['Singleton_Rate']:.2f}"
            print(f"║  {m} {cov} {sz} {sgl}")
        print("╚" + "═"*46 + "╝\n")

        # Save and Plot
        self.save_and_plot(cp_metrics, cp_results, dates_test, alpha_sequence, covered_seq, y_test_arr)

    def evaluate_cp(self, prediction_sets, y_true, method_name):
        n = len(y_true)
        covered = [prediction_sets[i, y_true[i]] for i in range(n)]
        coverage = np.mean(covered)
        avg_set_size = np.mean(prediction_sets.sum(axis=1))
        singleton_rate = np.mean(prediction_sets.sum(axis=1) == 1)
        empty_rate = np.mean(prediction_sets.sum(axis=1) == 0)
        
        up_weeks = np.where(y_true == 1)[0]
        down_weeks = np.where(y_true == 0)[0]
        cov_up = np.mean([prediction_sets[i, 1] for i in up_weeks]) if len(up_weeks) > 0 else None
        cov_down = np.mean([prediction_sets[i, 0] for i in down_weeks]) if len(down_weeks) > 0 else None

        return {
            "Method": method_name, "Coverage": round(coverage, 4), "Avg_Set_Size": round(avg_set_size, 4),
            "Singleton_Rate": round(singleton_rate, 4), "Empty_Set_Rate": round(empty_rate, 4),
            "Coverage_UP": round(cov_up, 4) if cov_up is not None else None,
            "Coverage_DOWN": round(cov_down, 4) if cov_down is not None else None
        }

    def save_and_plot(self, cp_metrics, cp_results, dates_test, alpha_sequence, covered_seq, y_test_arr):
        import pandas as pd
        import matplotlib.pyplot as plt
        rbi_dates = pd.to_datetime([
            "2022-02-10", "2022-04-08", "2022-06-08", "2022-08-05",
            "2022-09-30", "2022-12-07", "2023-02-08", "2023-04-06",
            "2023-06-08", "2023-08-10", "2023-10-06", "2023-12-08",
            "2024-02-08", "2024-04-05", "2024-06-07", "2024-08-08",
            "2024-10-09", "2024-12-06", "2025-02-07", "2025-04-09",
        ])
        is_event_week = np.array([any(abs((d.tz_localize(None) - rbi).days) <= 7 for rbi in rbi_dates) for d in pd.to_datetime(dates_test)])
        
        event_results = []
        for m_name, p_sets in cp_results.items():
            cov_event = np.mean([p_sets[i, y_test_arr[i]] for i in np.where(is_event_week)[0]]) if sum(is_event_week)>0 else np.nan
            cov_nonevent = np.mean([p_sets[i, y_test_arr[i]] for i in np.where(~is_event_week)[0]]) if sum(~is_event_week)>0 else np.nan
            event_results.append({
                "Method": m_name, "Event_Coverage": cov_event, "NonEvent_Coverage": cov_nonevent,
                "Degradation": cov_nonevent - cov_event if not np.isnan(cov_event) and not np.isnan(cov_nonevent) else np.nan
            })
            print(f"{m_name}: Event coverage = {cov_event:.3f}, Non-event coverage = {cov_nonevent:.3f}, Degradation = {cov_nonevent - cov_event:.3f}")

        # CSVs
        pd.DataFrame(cp_metrics).to_csv(f"{self.results_dir}/cp_metrics_summary.csv", index=False)
        pd.DataFrame(event_results).to_csv(f"{self.results_dir}/cp_event_analysis.csv", index=False)
        pd.DataFrame({"Date": dates_test, "Alpha_t": alpha_sequence, "Covered": covered_seq}).to_csv(f"{self.results_dir}/cp_aci_alpha_sequence.csv", index=False)
        
        ps_df = pd.DataFrame({"Date": dates_test, "True_Label": y_test_arr})
        for m_name, p_sets in cp_results.items():
            cn = m_name.replace(" ", "_")
            ps_df[f"Pred_{cn}"] = [str(x) for x in p_sets]
            ps_df[f"Set_{cn}"] = p_sets.sum(axis=1)
        ps_df.to_csv(f"{self.results_dir}/cp_prediction_sets_test.csv", index=False)

        # Plot 1
        plt.figure()
        methods = [m['Method'] for m in cp_metrics]
        covs = [m['Coverage'] for m in cp_metrics]
        plt.bar(methods, covs, color=['green' if c>=0.9 else 'red' for c in covs])
        plt.axhline(0.9, color='red', linestyle='--')
        plt.savefig(f"{self.results_dir}/coverage_bar_chart.png")
        plt.close()

class EnsembleWrapper:
    def __init__(self, results_dict, weights):
    ...
    def predict_proba(self, X):
        ...
        for name, weight in self.weights.items():
            if name in self.results_dict and self.results_dict[name]['model'] is not None:
                model = self.results_dict[name]['model']
                try: p = model.predict_proba(X)[:, 1]
                except:
                    try: p = model.decision_function(X); p = (p - p.min()) / (p.max() - p.min())
                    except: p = np.zeros(len(X))
                ensemble_probs += weight * p
        ret = np.zeros((len(X), 2))
        ret[:, 1] = ensemble_probs
        ret[:, 0] = 1 - ensemble_probs
        return ret
...
<<<<<<< HEAD
    ...
    dates_calib = df_feat.index[train_end:calib_end]
...
<<<<<<< HEAD
    ...
    best_f1_model = results[best_f1_name]['model']
    print(f"\nUsing {best_f1_name} as base model for CQR (Highest F1).")
    
    wrapper = EnsembleWrapper(results, ensemble_result['weights'] if ensemble_result else ENSEMBLE_WEIGHTS)
    
    cp_module = ConformalPredictionModule()
    cp_module.run(
        ensemble_wrapper=wrapper,
